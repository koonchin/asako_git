...
const Hero: React.FC = () => {
  ...
  const [isVideoOpen, setIsVideoOpen] = useState(false); // ✅ State สำหรับ Video
          ...
          <div className="flex-1 space-y-8 animate-in fade-in slide-in-from-left-4 duration-1000">
            ...
            <div className="flex flex-wrap gap-4 pt-4">
              <button
                ...
                className="bg-gray-900 hover:bg-black text-white px-10 py-4 text-sm font-bold uppercase tracking-[0.2em] transition-all shadow-xl hover:shadow-2xl hover:-translate-y-1">
                {t('hero.view_catalog')}
              </button>
              {/* ✅ ปุ่มเปิดวิดีโอ */}
              <button 
                onClick={() => setIsVideoOpen(true)}
                className="border border-gray-300 hover:border-red-600 hover:text-red-600 text-gray-900 px-10 py-4 text-sm font-bold uppercase tracking-[0.2em] transition-all bg-white flex items-center gap-2">
                ▶ Watch Video
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 w-full relative h-[500px] lg:h-[600px] flex items-center justify-center">
             <div className="relative w-full h-full">
                ...
                <div className="absolute -bottom-10 -left-10 bg-white p-8 shadow-xl border-t-4 border-red-600 max-w-xs hidden md:block">
                   ...
                   {/* ✅ เปลี่ยนเป็น Experience */}
                   <p className="text-xs uppercase tracking-widest text-gray-500 mt-1">Years of Experience</p>
                </div>
             </div>
          </div>
        </div>
      ...
      {isVideoOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setIsVideoOpen(false)}>
          <div className="relative w-full max-w-5xl aspect-video bg-black rounded-lg overflow-hidden shadow-2xl">
            <button className="absolute -top-10 right-0 text-white hover:text-red-500 text-2xl font-bold" onClick={() => setIsVideoOpen(false)}>✕</button>
